...
class TelegramBot:
    ...
    async def send_candlestick_summary(self, df, timeframe):
        ...
        try:
            ...
            close_time = df['close_time'].max()  # Latest close_time in the DataFrame (in int timestamp)
            ...
            full_message += "Symbol / OC% / HL% / Z-OC / Z-HL / Z-Vol / Z-OC-All / Z-HL-All / Z-Vol-All\n"
            
            # Sort DataFrame by 'rate_change_open_close' in descending order
            df = df.sort_values(by='rate_change_open_close', ascending=False)

            # Add each symbol, rate changes, Z-scores, and cross-pair Z-scores to the message
            for _, row in df.iterrows():
                ...
                full_message += (
                    ...
                    f"{row['z_rate_change_open_close']:.2f} / "
                    f"{row['z_rate_change_high_low']:.2f} / "
                    f"{row['z_volume_pair']:.2f} / "
                    f"{row['z_rate_change_open_close_all_pairs']:.2f} / "
                    f"{row['z_rate_change_high_low_all_pairs']:.2f} / "
                    f"{row['z_volume_all_pairs']:.2f}\n"
                )

            # Send the entire message as a single Telegram message asynchronously
            await self.bot.send_message(chat_id=self.chat_id, text=f"{full_message}", parse_mode='Markdown')
